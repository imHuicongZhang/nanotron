...
1. Download the four data folders and the init checkpoints. Verify sha256 against manifest.json.
   ...
   - Init: `tools/hash_init_checkpoint.py --check` as above. That check covers the model weights only, so also confirm each `_init_1.5B_seed<S>/0/` holds all 180 files, including `optimizer/optimizer_pp-0-of-1_tp-0-of-1_exp-0-of-1.pt` (about 6.0 GB) and `lr_scheduler/lr_scheduler_pp-0-of-1_tp-0-of-1_exp-0-of-1.pt`. Every trunk1 loads them (`load_optimizer: true`); a missing optimizer file fails at load time (`FileNotFoundError`), after the job has been allocated. `plan_submit.py`'s seeding step refuses an incomplete init.
