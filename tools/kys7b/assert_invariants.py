#!/usr/bin/env python
"""Fail fast if the batch invariants have drifted — 7B grid.

7B fork of tools/assert_invariants.py. Same checks, same exit codes; the pinned mbs and the
corpus table differ. See tools/kys7b/README.md.

Two invariants hold across all 72 runs of the 7B grid, and both must be true or the runs are
not comparable to each other:

    micro_batch_size == 8                   (pinned for NUMERICAL comparability, not speed)
    mbs * accum * dp * seq_len == 2,097,152 tokens/step

The second is what most people check. The first is the one that actually gets broken, because
214.6 GiB of 268 GiB looks like headroom. It is not: `Loss.forward` computes
`masked_mean(loss, label_mask)` and `label_mask` drops the token before every document
boundary — measured 2.8 masked tokens per 2048-token sequence, range 0-10 — so masked_mean's
denominator varies per micro-batch and the effective per-token weight is `1/(n_micro * n_j)`.
Regrouping the same 1024 sequences into a different mbs moves those weights by ~1e-3 relative
(measured: 1.43e-3 for mbs=4 vs 16, 7.04e-4 for mbs=8 vs 16) — four orders of magnitude above
fp32 rounding. Nothing in a loss curve will look wrong; the runs are simply no longer
measuring the same objective. So mbs must be IDENTICAL across all 72 runs of this grid.

WHY 8 AND NOT 16 — and why that is not a problem. The 1.5B grid was actually run on B300 at
dp 4 with mbs 16 and accum 16. That is verified rather than assumed: read from
tokens.micro_batch_size in the config.yaml of the published 1.5B checkpoints (Phase 0,
2026-09-02; all three step directories agree) at
...
(~308 GiB estimated against a 241 GiB budget; see deploy/clusters_7b.yaml).

The masked_mean per-token weighting therefore differs between the 1.5B grid and the 7B grid
by up to ~7.04e-4 relative. This affects NO comparison the experiment makes. Every six-way
between-setting comparison is within ONE grid at ONE mbs, and the two grids already differ in
learning rate, warmup, token budget, embedding tying and parameter count — mbs is not what
makes them non-comparable, and they were never intended to be compared at the objective level.
Within the 7B grid, mbs 8 is pinned exactly as hard as mbs 16 was within the 1.5B grid.

Run it two ways:

  preflight (before any GPU time — this is the cheap one):
      python tools/kys7b/assert_invariants.py --config rendered/quality_base_seed42_trunk1.yaml

  smoke test (after ~200 steps of a real run, catches an edited config or a stale env):
      python tools/kys7b/assert_invariants.py --config rendered/x.yaml --log train.log --at-step 200
...
# `data_root` holds when it is a snapshot of the tokenized-corpora repo. See the legacy-name
# remap table in tools/kys7b/render_config.py if you are looking at an old /scratch tree.
#
# This is the check that catches a path which EXISTS but holds the wrong corpus. That failure
# is otherwise silent: nanotron starts, trains for weeks, and produces numbers that mean
# nothing.
#
...
#
# ALL SIX ARE UNFILLED PLACEHOLDERS. The 50B corpora have not been tokenized, so their token
# counts and shard counts do not exist yet. Each value must become a
# `(token_count, shard_count)` tuple once tokenization is done — token_count being the sum of
# line 2 of every *.ds.metadata in the folder, cross-checked against raw .ds bytes/2, exactly
# as the 1.5B counts in tools/assert_invariants.py were produced.
#
# A null here is NOT a soft warning: check_corpus_table() below refuses to pass while any
# value is None, and it runs unconditionally — there is deliberately no flag that skips it.
# A default token count would be far worse than a missing one, because it would let the grid
# start against an unverified corpus.
EXPECTED_CORPUS = {
    'quality_base':       None,   # PLACEHOLDER — fill in (token_count, shard_count) after tokenization
    'quality_first':      None,   # PLACEHOLDER — fill in (token_count, shard_count) after tokenization
    'diversity_oriented': None,   # PLACEHOLDER — fill in (token_count, shard_count) after tokenization
    'wrap_inspired':      None,   # PLACEHOLDER — fill in (token_count, shard_count) after tokenization
    'rewire_inspired':    None,   # PLACEHOLDER — fill in (token_count, shard_count) after tokenization
    'disagreement_aware': None,   # PLACEHOLDER — fill in (token_count, shard_count) after tokenization
}


def check_corpus_table():
    """Refuse to certify anything while the 50B corpus token counts are unfilled.

    Runs unconditionally — including under --skip-corpus, which only skips inspecting the
    files on disk. The point of --skip-corpus is "the corpora are not on this host"; it is not
    a way to declare an unverifiable grid verified. Until these counts exist there is no
    reference to compare a corpus against, so the token-count check cannot do its job and
    assert_invariants must not print OK.
    """
    unfilled = [k for k, v in EXPECTED_CORPUS.items() if v is None]
    if not unfilled:
        return []
    return ["EXPECTED_CORPUS has unfilled placeholders for: " + ", ".join(sorted(unfilled))
            + f"\n  ({len(unfilled)} of {len(EXPECTED_CORPUS)} settings). The 50B corpora have not "
              "been tokenized, so there is no token count to verify a dataset_folder against.\n"
              "  Fill each entry in tools/kys7b/assert_invariants.py with its real "
              "(token_count, shard_count) — token_count being the sum of line 2 of every "
              "*.ds.metadata in that corpus folder — and re-run.\n"
              "  Until then this grid cannot be preflighted and must not be started."]

# nanotron/trainer.py logs this banner once at startup:
#   mbs: 8 | grad_accum: 32 | cp: 1 | sequence_length: 2048 | global_batch_size: 1024 | ...
BANNER = re.compile(
    r"mbs:\s*(\d+)\s*\|\s*grad_accum:\s*(\d+)\s*\|\s*cp:\s*(\d+)\s*\|\s*"
    r"sequence_length:\s*(\d+)\s*\|\s*global_batch_size:\s*(\d+)"
)
# and per iteration:  iteration: 200 / 21458 | consumed_tokens: 419M | ...
...
def fail(msgs):
    ...
    print("  See deploy/clusters_7b.yaml and configs/know-your-sources-7b/README.md before\n  changing anything.", file=sys.stderr)
    sys.exit(1)
...
def check_corpus(cfg_path: Path):
    ...
    for f in folders:
        ...
        if exp is None:
            errs.append(f"{corpus}: EXPECTED_CORPUS entry is an unfilled PLACEHOLDER, so the "
                        f"{total:,} tokens found in {p} cannot be verified against anything. "
                        f"Fill in (token_count, shard_count) for {corpus!r} in "
                        f"tools/kys7b/assert_invariants.py.")
            continue
...
def check_resume(cfg_path: Path):
    ...
    nanotron proceeds from RANDOM INIT. A cooldown branch would run its 2384/4768/7153 steps,
...
def main():
    ...
    # Unconditional: not gated on --skip-corpus. See check_corpus_table().
    errs += check_corpus_table()
