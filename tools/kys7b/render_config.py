#!/usr/bin/env python
"""Stamp a per-cluster deployment profile onto an experiment template — 7B grid.

7B fork of tools/render_config.py: same guards, same derivation, 7B paths and profile.
See tools/kys7b/README.md for the constant-by-constant diff.

The experiment templates (configs/know-your-sources-7b/*.yaml) describe WHAT is being trained: model, data,
...
not experiment, and live only in deploy/clusters_7b.yaml.
...
Usage:
    python tools/kys7b/render_config.py \
        --template configs/know-your-sources-7b/quality_base_seed42_trunk1.yaml \
        --cluster b300 --seed 42 --out /tmp/rendered.yaml
    python tools/kys7b/render_config.py --template configs/know-your-sources-7b/... \
        --cluster b300 --seed 42 --print
"""
...
# This file lives at <repo>/tools/kys7b/, one directory deeper than tools/render_config.py,
# so the repo root is three parents up, not two.
HERE = Path(__file__).resolve().parent.parent.parent
DEFAULT_PROFILE = HERE / 'deploy' / 'clusters_7b.yaml'
...
#
# WHERE THE NAMES COME FROM. For the 7B grid `data_root` is expected to be a snapshot of
# wytro/Know-Your-Sources-7B-tokenized   # PLACEHOLDER — that repo does not exist yet. The 50B
#                                        # corpora have not been tokenized or uploaded; this is
#                                        # the intended name, not a verified one.
# Expected layout is <arm>/tokenized/*.ds under exactly the six names below, matching the 1.5B
# repo wytro/Know-Your-Sources-tokenized (verified against that live repo listing 2026-08-12:
# 6 arms x 16 .ds + .ds.index + .ds.metadata, plus a tokenizer/ directory). Shard counts for
# the 50B corpora are not yet known.
#
...
#
# This file describes the HuggingFace layout, NOT the legacy /scratch layout. Rendering
...
# tools/kys7b/assert_invariants.py then verifies each resolved path by token count before any
# GPU time is spent — but its EXPECTED_CORPUS table is all-null until the 50B corpora exist,
# and it refuses to pass while that is so.
# ---------------------------------------------------------------------------------------
SETTING_CORPUS = {
...
def main():
    ...
    if missing:
        die(f'cluster {args.cluster!r} profile is incomplete: {missing} are null. '
            f'Fill deploy/clusters_7b.yaml from the real node spec — do not assume 8 GPUs/node.')
    ...
    # ~1e-3 relative. See deploy/clusters_7b.yaml for the measurements.
    used = {}
    ...
    if kind.startswith('trunk'):
        ...
        # latest step (64373) instead of its own branch point. The step is the branch's own
    ...
    env_lines = [
        '# sourced by the launcher; written by tools/kys7b/render_config.py — do not edit',
    ...
    banner = (f'# RENDERED by tools/kys7b/render_config.py — do not edit.\n'
